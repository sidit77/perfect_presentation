use crate::{InteropState, SharedTexture, HWND};
use std::cell::Cell;
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn destroy_context(window: WindowIdentifier) -> i32 {
    GLFW_WINDOWS.lock().unwrap().remove(&window).expect("No context with that identifier");
    0
}
